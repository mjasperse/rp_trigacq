...
int process_sig(int *in_cha_signal, int *in_chb_signal, int dec_factor)
{
	...
	if (!data)
		data = calloc(SIGNAL_LENGTH, 3*sizeof(float));
	...
	for(t_idx=0; t_idx < SIGNAL_LENGTH; ++t_idx, ++in_idx)
	{
		...
		ptr[0] = t_idx * t_factor;
		ptr[1] = osc_fpga_cnv_cnt_to_v(in_cha_signal[in_idx], adc_max_v1, calibr.fe_ch1_dc_offs/*calib_dc_off*/, 0/*user_dc_off*/);
		ptr[2] = osc_fpga_cnv_cnt_to_v(in_chb_signal[in_idx], adc_max_v2, calibr.fe_ch2_dc_offs/*calib_dc_off*/, 0/*user_dc_off*/);
        ptr += 3;
	}
...
int acquire(int trig_chan, int decim_enum, int timeout)
{
	...
	int *localA, *localB;
	localA = calloc(SIGNAL_LENGTH,sizeof(int));
	localB = calloc(SIGNAL_LENGTH,sizeof(int));
	...
	// MEMCPY IS A FRIEND THAT HAS BETRAYED US (BUS ERROR?!)
	// memcpy(localA,ptrA,SIGNAL_LENGTH*sizeof(int));
	// memcpy(localB,ptrB,SIGNAL_LENGTH*sizeof(int));
...
{	
	...
	else
	{
		...
		// write to stdout
		fwrite(data,SIGNAL_LENGTH,3*sizeof(float),stdout);
	}
	osc_fpga_exit();
	return ret;
}
		...
		while (ret == 0)
		{
			...
			if (!strncmp(msg,"ACQ",3))
			{
				...
				if (ret > 0) 
				{
					...
					int nb = len * 3*sizeof(float);
					char *ptr = ( char* )data;
					while ((nb>0) && (ret>0))
					{
						// try to send a large data packet
						ret = send(fd, ptr, nb, 0);
						printf("Sent %p, %i, %i\n", ptr, nb, ret);
						// check how much actually got send, divide it up
						nb -= ret;
						ptr += ret;
					}
					if (ret < 0) perror("Failed to send data");
				}
